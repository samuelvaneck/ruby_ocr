...
window.addEventListener('DOMContentLoaded', () => {
  ...
  const getFormData = (btn) => {
    ...
    data.full_text = JSON.stringify(atob(document.getElementById('full-text').value))
...
// check top checkbox
window.addEventListener('DOMContentLoaded', () => {
  if (document.getElementById('top-checkbox') === null) { return }

  document.getElementById('top-checkbox').addEventListener('change', (event) => {
    const checkboxes = document.getElementsByName('line');
    Array.from(checkboxes).forEach((checkbox) => {
